...
Use `auto` for general code discovery, `text` for lexical evidence, `symbol` for definitions and references, `dependency` for the local dependency neighborhood, and `impact` before dependency-relevant edits to find dependents and related tests. In `text` mode, all unquoted terms are required; wrap adjacent terms in double quotes to require an exact phrase. Use `path` to restrict results to a repository-relative file or directory. Results cite the repository root, relative path, line range, matching snippet, score reason, relationship, and exact index generation.
